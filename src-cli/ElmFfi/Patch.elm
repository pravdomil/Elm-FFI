    ...
    a
        ...
        |> Result.map
            (String.replace
                ...
                "var $author$project$JavaScript$Decoder$bytes = __Json_decodePrim(function(a) { return a instanceof ArrayBuffer ? __Result_Ok(new DataView(a)) : __Json_expecting('a Buffer', a) });"
            )
        |> Result.map
            (String.replace
