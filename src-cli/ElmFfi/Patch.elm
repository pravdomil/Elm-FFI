    ...
    a
        ...
        |> Result.map
            (String.replace
                ...
                "var $author$project$JavaScript$Decoder$bytes = _Json_decodePrim(function(a) { return a instanceof ArrayBuffer ? __Result_Ok(new DataView(a)) : __Json_expecting('a Buffer', a) });"
            )
        |> Result.map
            (String.replace
        ...
        |> Result.map
            (String.replace
                ...
                "var $author$project$JavaScript$Decoder$timePosix = _Json_decodePrim(function(a) { return a instanceof Date ? __Result_Ok($elm$time$Time$millisToPosix(a.valueOf())) : __Json_expecting('a Date', a) });"
            )
        |> Result.map
            (String.replace
